...
#
# There are multiple ``TableLoader`` methods loading data from all relevant tables (depending
# on the options) and **joins** them into single tables:
#
...
#
# The last one returns a dict with a table per telescope type, which is needed for e.g. DL1
# image data that might have different shapes for each of the telescope types as tables do
# not support variable length columns.
#
# **It is recommended to use the** ``TableLoader`` **when loading data into Tables, because
# its much faster than EventSources!**
...
for id, table in telescope_events_by_id.items():
    print(f"Telescope ID: {id} \n", table, "\n")
