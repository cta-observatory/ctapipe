...
- hillas_parameters_1 and hillas_parameters_2 does not match. Mismatch in hillas_parameters_2 need to be resolved.
...
- Higher order correlations implemented.
- Sanity checks for size, x_y correlation, length, width.
- Parameter delta introduced : angle between ellipse major axis and x-axis. Delta defined as -pi/2 < delta < pi/2 .
- Correction in implementation of miss of hillas_parameters_1. Previous implementation missed a square root factor
- Implementation of Higher Order Moment Parameters : Skewness, Kurtosis.
- Implementation of Asymmetry. Definition same as MAGIC asym. Alternative definition of asym using highr order correlations mentioned in     
  comments below asym .
"""
...
MomentParameters = namedtuple(
    "MomentParameters",
    "size,cen_x,cen_y,length,width,r,phi,delta,miss"
)
...
def hillas_parameters_1(pix_x, pix_y, image):
    ...
    a = d2 / d1
    delta = np.arctan2(a)                        # Angle between ellipse major ax. and x-axis of camera. Will be used for disp
    b = mean_y - a * mean_x
    ...
    #asym3 = np.power(cos_delta, 3) * S_xxx + 3.0 * np.power(cos_delta, 2) * sin_delta * S_xxy + 3.0 * cos_delta * np.power(sin_delta, 2) * 
    #      S_xyy + np.power(sin_delta, 3) * S_yyy
    ...
    return MomentParameters(size=size, cen_x=mean_x, cen_y=mean_y, length=length, width=width, r=r, phi=phi, delta=delta, miss=miss), HighOrderMomentParameters (Skewness=skewness, Kurtosis=kurtosis, Asymmetry=asym)
