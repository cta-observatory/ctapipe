...
def test_muon_reconstruction(tmpdir):
    ...
    tool.run(argv=shlex.split(f"--input={GAMMA_TEST_LARGE} " "--max_events=2 "))
...
def test_display_summed_images(tmpdir):
    ...
    tool.run(argv=shlex.split(f"--infile={GAMMA_TEST_LARGE} " "--max-events=2 "))
...
def test_display_integrator(tmpdir):
    ...
    tool.run(argv=shlex.split(f"--f={GAMMA_TEST_LARGE} " "--max_events=1 "))
...
def test_display_dl1(tmpdir):
    ...
    tool.run(argv=shlex.split("--max_events=1 " "--telescope=11 "))
...
def test_extract_charge_resolution(tmpdir):
    ...
    with pytest.raises(KeyError):
        tool.run(["-f", GAMMA_TEST_LARGE, "-O", output_path])
