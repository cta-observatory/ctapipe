...
def eigvals_sym_2d(cov):
    ...
    if c == 0:
        ...
        psi = 0.5 * (np.arctan2(2 * c, diff))

    # normalise to [-np.pi / 2, np.pi / 2]
    if psi > (np.pi / 2):
        psi -= np.pi
    return np.array([l1, l2]), psi
...
@njit
def hillas_cov(x, y, weights):
    cov_xx = np.sum(x ** 2 * weights)
    cov_yy = np.sum(y ** 2 * weights)
    cov_xy = np.sum(x * y * weights)
    return np.array([[cov_xx, cov_xy], [cov_xy, cov_yy]])


@njit(cache=True)
def hillas_no_units(pix_x, pix_y, image):
    size = np.sum(image)
    weights = image / size
    ...
    cog_x = np.sum(pix_x * weights)
    cog_y = np.sum(pix_y * weights)

    # polar coordinates of the cog
    cog_r = np.sqrt(cog_x ** 2 + cog_y ** 2)
    ...
    cov = hillas_cov(delta_x, delta_y, weights)
    eig_vals, psi = eigvals_sym_2d(cov)
    ...
    if length == 0:
        ...
        m3_long = np.sum(longitudinal ** 3 * image / size)
        skewness = m3_long / length ** 3

        m4_long = np.sum(longitudinal ** 4 * image / size)
        kurtosis = m4_long / length ** 4
...
def hillas_parameters(geom, image):
    ...
    (
        ...
        skewness,
        kurtosis,
    ) = hillas_no_units(pix_x, pix_y, image)
