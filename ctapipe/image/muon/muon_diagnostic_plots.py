...
def plot_muon_efficiency(outputpath):
    ...
    axrw.set_ylim(0.0, 1.2 * max(contrw[0]))
    ...
    if muonparams["MuonRingParams"] is not None:
        ...
        for tel_id in muonparams["TelIds"]:
            ...
            ring_camcoord = ring_nominal.transform_to(
                CameraFrame(
                    pointing_direction=altaz, focal_length=flen, rotation=rotr_angle
                )
            )
            ...
            camera_coord = SkyCoord(
                x=px,
                y=py,
                frame=CameraFrame(focal_length=flen, rotation=geom.pix_rotation),
            )

            nom_coord = camera_coord.transform_to(NominalFrame(origin=altaz))
            ...
            dist = np.sqrt(
                (px - ring.ring_center_x) ** 2 + (py - ring.ring_center_y) ** 2
            )
            ...
            if muonparams["MuonIntensityParams"][idx] is not None:
                ...
                ringwidthfrac = (
                    muonparams["MuonIntensityParams"][idx].ring_width
                ...
                if len(pred) != np.sum(muonparams["MuonIntensityParams"][idx].mask):
                    logger.warning(
                        "Lengths do not match...len(pred)=%s len(" "mask)=",
                        len(pred),
