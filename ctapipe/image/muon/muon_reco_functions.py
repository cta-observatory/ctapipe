...
def analyze_muon_event(event):
    ...
    tail_cuts = [
        (5, 7), (5, 7), (10, 12), (5, 7), (5, 7), (5, 7), (5, 7), (5, 7)
    ]  # 10, 12?
    impact = [
        (0.2, 0.9), (0.1, 0.95), (0.2, 0.9), (0.2, 0.9), (0.1, 0.95),
        (0.1, 0.95), (0.1, 0.95), (0.1, 0.95)
    ] * u.m
    ringwidth = [
        (0.04, 0.08), (0.02, 0.1), (0.01, 0.1), (0.02, 0.1), (0.01, 0.5),
    ...
    ang_pixel_width = [
        0.1, 0.2, 0.18, 0.067, 0.24, 0.2, 0.17, 0.2, 0.163
    ] * u.deg
    ...
    for telid in event.dl0.tels_with_data:
        ...
        logger.debug(
            'found an index of %d for camera %d', dict_index, geom.cam_id
        )
        ...
        altaz = HorizonFrame(
            alt=altval, az=event.mcheader.run_array_direction[0]
        )
        ...
        logger.debug(
            "img: %s mask: %s, x=%s y= %s", np.sum(image), np.sum(clean_mask),
            x, y
        )
        ...
        if (
            ...
            if image.shape[0] == muon_cuts['total_pix'][dict_index]:
                ...
                if all(conditions):
                    ...
                    logger.debug(
                        "Muon found in tel %d,  tels in event=%d", telid,
