...
def load_hessio(filename):
    ...
    try:
        for t,tid in enumerate(tel_id):
            ...
            try:
                pix_posX = h.get_pixel_position(tid)[0]
                pix_id = np.arange(len(pix_posX))
                cam_table_prime['PixID'] = pix_id
                cam_table_prime['PixX'] = pix_posX
                cam_table_prime['PixX'].unit = u.m
            ...
            try:
                pix_posY = h.get_pixel_position(tid)[1]
                cam_table_prime['PixY'] = pix_posY
                cam_table_prime['PixY'].unit = u.m
            except: pass
            try:
            ...
            try:
                opt_table_prime.meta['MirNum'] = h.get_mirror_number(tid)
            except: pass
            try:
                opt_table_prime['MirArea'] = h.get_mirror_area(tid)
                opt_table_prime['MirArea'].unit = u.m**2
                opt_table_prime.meta['MirAreaDescription'] =\
                'Area of all mirrors'
            except: pass
            try:
                opt_table_prime['OptFocLen'] = h.get_optical_foclen(tid)
