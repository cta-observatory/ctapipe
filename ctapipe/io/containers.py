...
class DL1CameraContainer(Container):
    ...
    extracted_samples = Field(None, ("numpy array of bools indicating which "
                                    "samples were included in the "
                                    "charge extraction as a result of the "
                                    "charge extractor chosen. "
                                    "Shape=(nchan, npix, nsamples)."))
    peakpos = Field(None, ("numpy array containing position of the peak as "
                          "determined by the "
                          "peak-finding algorithm for each pixel and channel"))
    cleaned = Field(None, ("numpy array containing the waveform "
                          "after cleaning"))
...
class DL1Container(Container):
    ...
    tel = Field(Map(DL1CameraContainer),
...
class MCCameraEventContainer(Container):
    ...
    """
    photo_electron_image = Field(Map(), ("reference image in pure "
    ...
    dc_to_pe = Field(None, "DC/PE calibration arrays from MC file")
    pedestal = Field(None, "pedestal calibration arrays from MC file")
    azimuth_raw = Field(0, "Raw azimuth angle [radians from N->E] "
                          "for the telescope")
    altitude_raw = Field(0, "Raw altitude angle [radians] for the telescope")
    azimuth_cor = Field(0, "the tracking Azimuth corrected for pointing "
                          "errors for the telescope")
    altitude_cor = Field(0, "the tracking Altitude corrected for pointing "
                           "errors for the telescope")
...
class MCHeaderContainer(Container):
    ...
    """
    run_array_direction = Field([], "the tracking/pointing direction in "
...
class ReconstructedShowerContainer(Container):
    ...
    core_y = Field(0.0, 'reconstructed y coordinate of the core position',
                   unit=u.m)
    core_uncert = Field(0.0, 'uncertainty of the reconstructed core position',
                        unit=u.m)
...
class ParticleClassificationContainer(Container):
    ...
    tel_ids = Field([], ('array containing the telescope ids used '
