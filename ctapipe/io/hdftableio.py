...
class HDF5TableWriter(TableWriter):
    ...
    num_rows_expected: int
        an estimate of how many rows will be written to the tables in the
        file, which helps pytables choose a correct chunksize. A more
        accurate value will improve performance.

    """

    def __init__(self, filename, group_name, num_rows_expected=1000, **kwargs):
        super().__init__()
        self._schemas = {}
        self._tables = {}
        self._h5file = tables.open_file(filename, mode="w", **kwargs)
        self._group = self._h5file.create_group("/", group_name)
        self._num_rows_expected = num_rows_expected
    ...
    def _setup_new_table(self, table_name, container):
        ...
        table = self._h5file.create_table(where=self._group,
                                          ...
                                          description=self._schemas[
                                              table_name],
                                          expectedrows=self._num_rows_expected)
