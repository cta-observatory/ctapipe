...
class SimTelEventSource(EventSource):
    ...
    def __init__(self, config=None, tool=None, **kwargs):
        ...
        self.file_ = eventio.simtel.simtelfile.SimTelFile(self.input_url)
    ...
    def prepare_subarray_info(self):
        ...
        for tel_id, cam_settings in self.file_.cam_settings:
    ...
    def _generator(self):
        ...
        for counter, (shower, event) in enumerate(self.file_):
            ...
            tels_with_data = set(self.file_.header['tel_id'])
            data.count = counter
            ...
            cent_event = event['event']['cent_event'].parse_data_field()
            MC_EVENT = event['mc_event'].parse_data_field()
            SHOWER = shower.parse_data_field()
            ...
            for tel_id in tels_with_data:
                H = event['event']['tel_events'][tel_id]['header'].parse_data_field()
                PL = event['event']['tel_events'][tel_id]['pixel_list'].parse_data_field()
                ...
                data.mc.tel[tel_id].photo_electron_image = event['pe_sum'].parse_data_field()['num_pe'][tel_id-1]
                ...
                data.mc.tel[tel_id].azimuth_raw = event['event']['tel_events'][tel_id]['track']['alitude_raw']
                try:
                    data.mc.tel[tel_id].azimuth_cor = event['event']['tel_events'][tel_id]['track']['azimuth_cor']
                    data.mc.tel[tel_id].altitude_cor = event['event']['tel_events'][tel_id]['track']['alitude_cor']
                except ValueError:
