...
class DataWriter(Component):
    ...
    write_waveforms = Bool(
        help="Store DL0 waveforms if available", default_fill_value=False
    ).tag(config=True)
    ...
    def __call__(self, event: ArrayEventContainer):
        ...
        if self.write_waveforms:
            self._write_dl0_telescope_events(self._writer, event)
    ...
    def _setup_writer(self):
        ...
        if self.transform_waveform:
            ...
            writer.add_column_transform_regexp(
                "dl0/event/telescope/.*", "waveform", transform
            )
    ...
    def _write_dl0_telescope_events(
        self, writer: TableWriter, event: ArrayEventContainer
    ):
        for tel_id, dl0_tel in event.dl0.tel.items():
            ...
            dl0_tel.prefix = ""
            writer.write(f"dl0/event/telescope/{table_name}", [tel_index, dl0_tel])
