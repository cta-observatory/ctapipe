...
from ctapipe.io.dstio import dst_event_source, dst_get_list_event_ids
...
class HessioFileReader(EventFileReader):
    ...
    def check_file_compatibility(file_path):
        ...
        if file_path.endswith('.gz') and 'dst' not in file_path:
            compatible = True
        return compatible
...
class DstioFileReader(EventFileReader):
    name = 'DstioFileReader'
    origin = 'dstio'

    @staticmethod
    def check_file_compatibility(file_path):
        compatible = False
        if file_path.endswith('.gz') and 'dst' in file_path:
            compatible = True
        return compatible
    ...
    def event_id_list(self):
        ...
        if self._event_id_list:
            ...
            self.log.info("Building new list of event ids...")
            ids = dst_get_list_event_ids(self.input_path,
                                         max_events=self.max_events)
            self._event_id_list = ids
        self.log.info("List of event ids retrieved.")
        return self._event_id_list
    ...
    def read(self, allowed_tels=None, requested_event=None,
        ...
        if self.max_events:
            self.log.info(
                "Max events being read = {}".format(self.max_events))
        source = dst_event_source(self.input_path,
                                  max_events=self.max_events,
                                  allowed_tels=allowed_tels,
                                  requested_event=requested_event,
                                  use_event_id=use_event_id)
