...
def test_barrel_distortion():
    """Test the case for no distortion on a square grid of points."""
    from ctapipe.coordinates.camera_frame import barrel_distortion

    x = np.linspace(0, 2, 10)
    y = np.linspace(0, 2, 10)
    x_new, y_new = barrel_distortion(x, y, 0)
    assert (x_new.all(), y_new.all()) == (x.all(), y.all())


def test_apply_coma_correction():
    """Test the case for no correction on LSTCam."""
    from ctapipe.coordinates.camera_frame import apply_coma_correction
    from ctapipe.instrument import CameraGeometry

    geom = CameraGeometry.from_name("LSTCam")
    x = geom.pix_x
    y = geom.pix_y
    x_coma, y_coma = apply_coma_correction(x, y, 0)
    assert (x_coma.value.all(), y_coma.value.all()) == (x.value.all(), y.value.all())
...
def test_telescope_separation():
    ...
    tel1 = SkyCoord(delta_az=0 * u.deg, delta_alt=0 * u.deg, frame=telescope_frame)
    tel2 = SkyCoord(delta_az=0 * u.deg, delta_alt=1 * u.deg, frame=telescope_frame)
