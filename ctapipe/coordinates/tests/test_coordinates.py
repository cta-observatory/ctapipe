...
def test_cam_to_tel():
    ...
    # check rotation
    telescope_coord_rot = camera_coord.transform_to(TelescopeFrame(focal_length = 15*u.m,rotation=90*u.deg))
    assert telescope_coord_rot.y[0] == (1./15.)*u.rad
...
def test_ground_to_tilt():
    ...
    # Check that if we look at horizoon the x coordinate is 0
