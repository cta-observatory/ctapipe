...
class PipeStep():
    ...
    """
	def __init__(self,section_name, prev_step=None,port_in=None, port_out=None,nb_thread=1):
	...
	def  __repr__(self):
		if self.prev_step != None:
			...
			return "WARNING prev_step==None  Name[ "+str(self.section_name) + " ], previous step[ " + " port in[ " + str(self.port_in)  + " ], port out [ " + str(self.port_out) +" ]"
...
class StepInfo():
	...
	def __init__(self,step_zmq,name=None):
		...
		if step_zmq != None:
			...
			if isinstance(step_zmq,RouterQueue):self.type = self.ROUTER
		else:
			self.name = 'ND'
			self.type = 'ND'
		self.queue_size = 0
	...
	def __repr__(self):
		return self.name + ", " + self.type + ", " + str(self.running) + ", " + str(self.queue_size)
...
class Pipeline(Tool):
	...
	gui_address = Unicode('localhost:5565', help='GUI adress and port').tag(config=True, allow_none=True)

	#producer_conf = Dict(default_value={"name": "PRODUCER", "class": "Producer", "module": "producer"},
	producer_conf = Dict(
	help='producer description: name , module, class',
						allow_none=False).tag(config=True)

	#stagers_conf = List(default_value=[{"name": "STAGE", "class": "Stager", "module": "stager" ,"nb_thread" : 2},
	#{"name": "STAGE2", "class": "Stager2", "module": "stager2" ,"nb_thread" : 1}],
	stagers_conf = List(
	help='stagers list description in a set order:',allow_none=False).tag(config=True)

	consumer_conf = Dict(default_value={"name": "CONSUMER", "class": "Producer", "module": "producer",  "prev" : "STAGE1"  },
	help='producer description: name , module, class',	allow_none=False).tag(config=True)
	...
	NB_THREAD = 'nb_thread'
	GUI_GRAPH           ="GUI_GRAPH"
	GUI_ROUTER_CHANGE   ="GUI_ROUTER_CHANGE"
	GUI_STAGER_CHANGE   ="GUI_STAGER_CHANGE"
	GUI_CONSUMER_CHANGE ="GUI_CONSUMER_CHANGE"
	...
	def init(self):
		...
		for producer_step in self.producer_steps:
			#conf = self.config['Pipeline']['producer_conf']
			conf = self.producer_conf
			producer_zmq = self.instantiation( producer_step.section_name,self.PRODUCER,port_out = producer_step.port_out,config=conf)
			if producer_zmq.init() == False :
				print("producer_zmq init failed", file=stderr)
				return False
			self.producer = producer_zmq

		# ROUTER
		#sock_router_port,socket_dealer_port,name="RDP",gui_address = None):
		...
		for consumer_step in self.consumer_step:
			...
			#router_port_out = str(int(consumer_step.port_in)+100)
			router_port_out = consumer_step.section_name + "_router"
			#router = RouterQueue( consumer_step.port_in,router_port_out,name = consumer_step.section_name + "_" + "router",gui_address=self.gui_address)
			#consumer_step.router_thread = router
			...
			#conf = self.config['Pipeline']['consumer_conf']
			...
			if consumer_zmq.init() == False :
				print("consumer_zmq init failed", file=stderr)
				return False
		...
		for stager_step in self.stager_steps:
			...
			#router_port_out = str(int(stager_step.port_in)+100)
			router_port_out = stager_step.section_name + "_router"
			#router = RouterQueue( stager_step.port_in,router_port_out, name = stager_step.section_name+"_router",gui_address=self.gui_address)
			#self.router_queues.append(router)
			...
			for i in range(stager_step.nb_thread):
				...
				stager_zmq = self.instantiation(stager_step.section_name ,self.STAGER,
												port_in = router_port_out,
												port_out = stager_step.port_out,
												name = stager_step.section_name + "$$thread_number$$"+ str(i),
												config=conf)
				if stager_zmq.init() == False :
					print("stager_zmq init failed", file=stderr)
					return False
				self.stagers.append(stager_zmq)
				stager_step.threads.append(stager_zmq)
			#stager_step.router_thread = router
		router = RouterQueue( sock_router_ports,socket_dealer_ports,step_names=router_names, gui_address=self.gui_address)
		if router.init() == False: return False
		self.router_queues.append(router)
	...
	def instantiation(self,section_name, stage_type, port_in = None, port_out=None,config=None,name=""):
		"""
		...
		obj = dynamic_class_from_module(class_name, module, configuration=config)
		...
		if stage_type == self.STAGER:
			thread = StagerZmq(obj,port_in,port_out,name=name,gui_address=self.gui_address)
			if  isinstance(obj, Coroutine):
				obj.set_socket(thread.get_output_socket())
		elif stage_type == self.PRODUCER:
			thread = ProducerZmq(obj,port_out,"producer",gui_address=self.gui_address)
			if  isinstance(obj, Coroutine):
				obj.set_socket(thread.get_output_socket())
		elif stage_type == self.CONSUMER:
			thread = ConsumerZmq(obj,port_in, "consumer",parent=self,gui_address=self.gui_address)
		...
		for section in self.conf.get_section_list():
			if self.conf.has_key( self.ROLE, section) == True:
				if self.conf.get(self.ROLE, section ) == role:
					if self.conf.has_key( self.NB_THREAD, section) == True:
						nb_thread = int(self.conf.get( self.NB_THREAD, section))
					else:
						nb_thread = 1
					if role == self.PRODUCER:
						step = PipeStep(section)
						step.type = self.PRODUCER
					elif role == self.STAGER:
						step = PipeStep(section,nb_thread = nb_thread)
						step.type = self.STAGER
					elif role == self.CONSUMER:
						step = PipeStep(section)
						step.type = self.CONSUMER
					result.append(step)
		"""
	...
	def def_step_for_gui(self):
		...
		self.levels_for_gui.append([StepInfo(self.router_thread,name=consumer.section_name+"_router")])
		prev = consumer.prev_step
		while prev != None:
			stages = list()
			for t in prev.threads:
			...
			if len(stages) > 0:
				...
				self.levels_for_gui.append([StepInfo(self.router_thread,name=prev.section_name+"_router")])
			prev = prev.prev_step
	...
	def get_step_by_section_name(self,section_name):
		...
		"""
		for step in self.producer_steps + self.stager_steps + self.consumer_step:
	...
	def start(self):
		...
		# Now send stop to thread and wait they join(when their queue will be empty)
	...
	def finish(self):
		self.log.info("Pipeline finish method call")
	...
	def wait_and_send_levels(self,thread_to_wait,conf_time):
		...
		while True:
			...
			self.socket_pub.send_multipart([b'GUI_GRAPH',pickle.dumps([conf_time,self.levels_for_gui])])
			if not thread_to_wait.is_alive() : break
	...
	def get_step_conf(self,name):
		...
		"""
		if self.producer_conf["name"] == name: return self.producer_conf
		if self.consumer_conf["name"] == name: return self.consumer_conf
