...
class RouterRep(FigureRep):
    ...
    def draw(self,qpainter,zoom=1):
        ...
        pensize=3
        qpainter.setPen(QtGui.QPen(QtCore.Qt.black,pensize, QtCore.Qt.SolidLine))
        ...
        qpainter.setPen(QtGui.QPen(PipelineDrawer.blue_cta, 3, QtCore.Qt.SolidLine))
...
class StagerRep(FigureRep):
    ...
    def draw(self,qpainter,zoom=1):
        ...
        size_y = self.size_y * zoom
        pensize = 3
        qpainter.setPen(QtGui.QPen(PipelineDrawer.blue_cta, pensize, QtCore.Qt.SolidLine))
        text_pos = QPointF(self.center)
        ...
        if self.running == True:
            qpainter.setPen(QtGui.QPen(PipelineDrawer.mygreen, 3, QtCore.Qt.SolidLine))
        else: qpainter.setPen(QtGui.QPen(PipelineDrawer.blue_cta, 3, QtCore.Qt.SolidLine))
...
class PipelineDrawer(QtGui.QWidget):
    ...
    def drawPipeline(self, qp):
        ...
        if len(self.levels) != 0:
            total_height = (len(self.levels) -1) * (STAGE_SIZE_Y + ROUTER_SIZE_Y)
        ...
        for level in self.levels:
            total_x = len(level) * STAGE_SIZE_X *self.zoom+ ( len(level)-1 * STAGE_GAP_X*self.zoom)
            ...
            for level in receiv_levels:
                ...
                for step in level:
                    if step.type == StepInfo.ROUTER:
                        steps.append(RouterRep(name=step.name,queue= step.queue_size))
                    elif step.type == StepInfo.STAGER:
                        steps.append(StagerRep(running=False,nb_job_done=
                        step.nb_job_done,fig_type=step.type, name = step.name,
