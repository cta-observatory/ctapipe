...
def test_invalid_events(subarray_and_event_gamma_off_axis_500_gev):
    ...
    for tel_id, dl1 in event.dl1.tel.items():
        ...
        mask = tailcuts_clean(geom, dl1.image, picture_thresh=10.0, boundary_thresh=5.0)
    ...
    for event in source:
        ...
        for tel_id, dl1 in event.dl1.tel.items():
            ...
            try:
                ...
                if (moments_camera_frame.width.value > 0) and (
                    moments_telescope_frame.width.value > 0
                ):
        ...
        if (len(hillas_dict_camera_frame) > 2) and (
            len(hillas_dict_telescope_frame) > 2
        ):
