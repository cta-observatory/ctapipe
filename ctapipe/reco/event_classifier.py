...
class EventClassifier(RegressorClassifierBase):
    ...
    def level_populations(self, group_signal, group_bgd, signal_evts, bgd_evts):
        ...
        try:
            ...
            raise TypeError(
                "This function wants pandas DataFrameGroupBy "
        ...
        for key in s:
            if key in group_signal.indices and key in group_bgd.indices:
                ...
                if exceeding > 0:
                    logger.debug(
                        "bin: %s - g: %s\th: %s\t - Removing %s " "protons",
                        key,
                    ...
                    r_ind_list = list(
                        np.random.choice(
                            group_bgd.indices[key], size=exceeding, replace=False
                        )
                    )
                    dfh.drop(r_ind_list, inplace=True)
                elif exceeding < 0:
                    logger.debug(
                        "bin: %s - g: %s\th: %s\t - Removing %s " "gammas",
                        key,
                        len(group_signal.indices[key]),
                        len(group_bgd.indices[key]),
                        np.abs(exceeding),
                    ...
                    r_ind_list = list(
                        np.random.choice(
                            group_signal.indices[key], size=-exceeding, replace=False
                        )
                    )
