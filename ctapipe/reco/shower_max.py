...
class ShowerMaxEstimator:
    def __init__(self, filename, col_altitude=0, col_thickness=2):
        """ small class that calculates the height of the shower maximum
            ...
            -----------
            filename : string
                path to text file that contains a table of
    ...
    def _interpolate(self, arg, outlierValue=0., order=3):
        ...
        return ndimage.map_coordinates(
            self.atmosphere.hist, [[coordinate]],
            order=order, cval=outlierValue)[0] * self.atmosphere.hist.unit
    ...
    def find_shower_max_height(self, energy, h_first_int, gamma_alt):
        ...
        gamma_alt : astropy.Quantity or float
            altitude / pi-minus-zenith (in radians in case of float) of
            the parent gamma photon
        ...
        c *= 36.62 * u.g * u.cm**-2
        # showers with a more horizontal direction spend more path length
        # in each atm. layer
        ...
        # total thickness at shower maximum = thickness at first interaction
        # + thickness traversed to shower maximum
        ...
        for ii, thick1 in enumerate(self.atmosphere.hist):
            if t_shower_max > thick1:
                ...
                thick2  = self.atmosphere.get_value([height2.to(
                    self.atmosphere.bin_lower_edges[0].unit).value])[0]
