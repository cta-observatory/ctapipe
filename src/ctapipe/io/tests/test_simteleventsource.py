...
def test_pointing():
    with SimTelEventSource(
        ...
        for array_event in reader:
            assert np.isclose(
                array_event.monitoring.pointing.altitude.to_value(u.deg), 70
            )
            assert np.isclose(
                array_event.monitoring.pointing.azimuth.to_value(u.deg), 0
            )
            assert np.isnan(array_event.monitoring.pointing.ra)
            assert np.isnan(array_event.monitoring.pointing.dec)

            # normal run, all telescopes point to the array direction
            for tel_event in array_event.tel.values():
                assert u.isclose(
                    tel_event.monitoring.pointing.azimuth,
                    array_event.monitoring.pointing.azimuth,
                )
                assert u.isclose(
                    tel_event.monitoring.pointing.altitude,
                    array_event.monitoring.pointing.altitude,
                )
...
def test_skip_r1_calibration():
    ...
    with SimTelEventSource(
        ...
        for event in reader:
            ...
            r1 = event.tel[1].r1
            np.testing.assert_(
                np.issubdtype(r0.waveform.dtype, np.uint16),
                f"R0 waveforms dtype is {r0.waveform.dtype}, expected uint16.",
            )
            np.testing.assert_(
                np.issubdtype(r1.waveform.dtype, np.float32),
                f"R1 waveforms dtype is {r1.waveform.dtype}, expected float32.",
            )
...
def test_skip_r1_calibration_with_gain_selection():
    ...
    with SimTelEventSource(
        ...
        for event in reader:
            ...
            msg = f"R0 waveforms should have 2 channels, got {r0.waveform.shape[0]}"
            assert r0.waveform.shape[0] == 2, msg
            # R1 should have 1 channel after gain selection
            assert r1.waveform.ndim == 3
            msg = f"R1 waveforms should have 1 channel after gain selection, got {r1.waveform.shape[0]}"
            assert r1.waveform.shape[0] == 1, msg
            # Check that selected_gain_channel exists and has correct shape
            assert r1.selected_gain_channel is not None
            msg = "selected_gain_channel should have one entry per pixel"
            assert r1.selected_gain_channel.shape[0] == r1.waveform.shape[1], msg
    assert n_processed == n_expected
...
def test_time_shift():
    ...
    for event in source:
        for tel_event in event.tel.values():
            ...
            assert np.any(calib.time_shift != 0 * u.s)
        ...
        for e in source:
            ...
            if event_id in strange_trigger_events:
                ...
                np.testing.assert_equal(
                    e.dl0.trigger.tels_with_trigger, [expected_tel_id]
                )
                assert e.tel.keys() == {expected_tel_id}
                events_checked_trigger.add(event_id)

            if event_id in missing_true_images:
                tel_id = missing_true_images[event_id]
                np.testing.assert_equal(e.tel[tel_id].simulation.true_image, -1)
...
def test_has_true_image_first_missing():
    ...
    with SimTelEventSource(input_url) as source:
        n_found = 0
        for event in source:
            assert event.tel[1].simulation.true_image is not None
            n_found += 1
        assert n_found == 2
