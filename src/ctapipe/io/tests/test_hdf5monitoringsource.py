...
def test_get_monitoring_types(
    ...
    assert tuple([]) == get_hdf5_monitoring_types(dl1_file)
...
def test_filling(dl1_file, dl1_merged_monitoring_file):
    ...
    with HDF5EventSource(
        input_url=dl1_file, allowed_tels=allowed_tels, max_events=1
    ) as source:
        monitoring_source = HDF5MonitoringSource(
...
def test_exceptions(dl1_file, calibpipe_camcalib_same_chunks):
