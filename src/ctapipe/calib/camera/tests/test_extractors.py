...
def test_extractors(example_subarray):
    ...
    sigmaclipping_extractor = SigmaClippingExtractor(subarray=example_subarray, sample_size=2500)
    ...
    assert np.any(np.abs(plain_stats_list[1].median - 2.0) > 1.5) == False
    assert np.any(np.abs(sigmaclipping_stats_list[1].median - 77.0) > 1.5) == False
    
    assert np.any(np.abs(plain_stats_list[0].std - 5.0) > 1.5) == False
    assert np.any(np.abs(sigmaclipping_stats_list[0].std - 10.0) > 1.5) == False
...
def test_check_outliers(example_subarray):
    ...
    assert sigmaclipping_stats_list[0].median_outliers[0][120] == True
    assert sigmaclipping_stats_list[0].median_outliers[1][67] == True
    assert sigmaclipping_stats_list[1].median_outliers[0][120] == True
    assert sigmaclipping_stats_list[1].median_outliers[1][67] == True
